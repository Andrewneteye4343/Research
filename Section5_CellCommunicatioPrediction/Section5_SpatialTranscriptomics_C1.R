...
# C1_norm.rds can be acquired by following the code in Section2_SpatialTranscriptomics_DataProcessing.R
C1_norm = readRDS(file = "path/to/V10F24-110_C1/C1_norm.rds")
data.input = GetAssayData(C1_norm, slot = "data", assay = "SCT")

# manually create a dataframe consisting of the cell labels
meta = data.frame(seurat_clusters = Idents(C1_norm), row.names = names(Idents(C1_norm))) 
meta$label = ""
meta$label[meta$seurat_clusters == "0"] = "AEP" # Alveolar epithelial cell-enriched region
meta$label[meta$seurat_clusters == "1"] = "Immune" # Immune cell-enriched region
meta$label[meta$seurat_clusters == "2"] = "AEP" # Alveolar epithelial cell-enriched region
meta$label[meta$seurat_clusters == "3"] = "CE" # Ciliated epithelial cell-enriched region
spatial.locs = GetTissueCoordinates(C1_norm, scale = NULL, cols = c("imagerow", "imagecol")) 
scale.factors = jsonlite::fromJSON(txt = file.path("path/to/V10F24-110_C1/spatial/", 'scalefactors_json.json'))
...
# This is the end of the Cell-cell Communication Prediction in Mendeley Data (DOI:10.17632/xjtv62ncwr.1) C1 slide
#################################################################################################################
