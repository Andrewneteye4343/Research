...
# Load the dataset
# GSE128033_norm.rds can be acquired by following the code in Section1_scRNA_snRNA_DataProcessing.R
GSE128033_norm = readRDS(file = "path/to/GSE128033/GSE128033_norm.rds")

# Set the default expression data into "RNA"
DefaultAssay(GSE128033_norm) <- "RNA"

# Seperate GSE128033_norm into control and IPF group
control = subset(GSE128033_norm, subset = group == "Control")
IPF = subset(GSE128033_norm, subset = group == "IPF")

# Create cellchat in each group
cellchat1 = createCellChat(object = control, group.by = "label")
cellchat2 = createCellChat(object = IPF, group.by = "label")
...
# This is the end of the Cell-cell Communication Prediction in GSE128033
#################################################################################################################