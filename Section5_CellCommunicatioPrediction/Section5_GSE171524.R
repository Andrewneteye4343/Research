...
# Reorder the label in GSE171524_norm
GSE171524_norm$label <- factor(
  GSE171524_norm$label,
  levels = c("AEP1","AEP2","B/DC","CD14CD16-Mo","CE","Cycling","ED","FB","HYP-M","Mast","Myo-FB",
             "Mucous","NA-M","Neuron","PF-M","PC","SMC","TR-M","Tcell","VCAN-M")
)

# Seperate GSE171524_norm into control and COVID-1 group
control = subset(GSE171524_norm, subset = group == "Control")
COVID_19 = subset(GSE171524_norm, subset = group == "COVID-19")

# Create cellchat in each group
cellchat1 = createCellChat(object = control, group.by = "label")
cellchat2 = createCellChat(object = COVID_19, group.by = "label")
...
incoming_heatmap_1 = netAnalysis_signalingRole_heatmap(object.list[[i]], pattern = "incoming", signaling = pathway.union, title = names(object.list)[i], width = 5, height = 6)
incoming_heatmap_2 = netAnalysis_signalingRole_heatmap(object.list[[i+1]], pattern = "incoming", signaling = pathway.union, title = names(object.list)[i+1], width = 5, height = 6)

# This is the end of the Cell-cell Communication Prediction in GSE171524
#################################################################################################################
