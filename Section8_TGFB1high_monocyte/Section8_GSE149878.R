...
monocyte = subset(GSE149878_norm, subset = label == "CD14CD16-Mo")

# Check the expression of TGFB1 in monocyte data
# We can find the mean value of TGFB1 in monocyte data is 0.6107
summary(FetchData(object = monocyte, vars = "TGFB1"))

# Define cells with lower TGFB1 expression than 0.6107 in monocyte data are TGFB1.neg
monocyte$TGFB1.group <- "TGFB1.pos"
monocyte$TGFB1.group[WhichCells(monocyte, expression = TGFB1 < 0.6107)] <- "TGFB1.neg"
...
# We can find the median value of TGFB1 in monocyte_2 data is 0.9122
summary(FetchData(object = monocyte_2, vars = "TGFB1"))

# Define cells with lower TGFB1 expression than 0.9122 in monocyte data are TGFB1.neg
monocyte_2$TGFB1.group <- "TGFB1.pos"
monocyte_2$TGFB1.group[WhichCells(monocyte_2, expression = TGFB1 < 0.9122)] <- "TGFB1.neg"

# Add the "TGFB1.group" label back to GSE149878_norm
...
GSE149878_norm$TGFB_label = ""

for (i in 1:length(allcellname)){
  if (allcellname[i] %in% colnames(group1)){
    GSE149878_norm$TGFB_label[i] = "TGFB1high_CD14CD16-Mo"
  }
  else if (allcellname[i] %in% colnames(group2)){
    GSE149878_norm$TGFB_label[i] = "TGFB1low_CD14CD16-Mo"
  }
  else if (allcellname[i] %in% colnames(group3)){
    GSE149878_norm$TGFB_label[i] = "TGFB1low_CD14CD16-Mo"
  }
  else {
    GSE149878_norm$TGFB_label[i] = GSE149878_norm$label[i]
  }
}

# Select the TGFB1high_CD14CD16-Mo and TGFB1low_CD14CD16-Mo cells
monocyte_3 = subset(x = GSE149878_norm, subset = TGFB_label == "TGFB1high_CD14CD16-Mo" | TGFB_label == "TGFB1low_CD14CD16-Mo")
...
vp_case1(gene_signature = gene_sig, file_name = "path/to/YourDirectory/gene_sig", 
         test_sign = comparisons, y_max = 3.5)
...
# This is the end of the TGFB1high monocyte in GSE149878
#################################################################################################################