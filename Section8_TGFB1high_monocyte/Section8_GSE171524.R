...
monocyte = subset(GSE171524_norm, subset = label == "CD14CD16-Mo")

# Check the expression of TGFB1 in monocyte data
# We can find the mean value of TGFB1 in monocyte data is 0.4226
summary(FetchData(object = monocyte, vars = "TGFB1"))

# Define cells with lower TGFB1 expression than 0.4226 in monocyte data are TGFB1.neg
monocyte$TGFB1.group <- "TGFB1.pos"
monocyte$TGFB1.group[WhichCells(monocyte, expression = TGFB1 < 0.4226)] <- "TGFB1.neg"
...
# We can find the median value of TGFB1 in monocyte_2 data is 1.9868
summary(FetchData(object = monocyte_2, vars = "TGFB1"))

# Define cells with lower TGFB1 expression than 1.9868 in monocyte data are TGFB1.neg
monocyte_2$TGFB1.group <- "TGFB1.pos"
monocyte_2$TGFB1.group[WhichCells(monocyte_2, expression = TGFB1 < 1.9868)] <- "TGFB1.neg"

# Add the "TGFB1.group" label back to GSE171524_norm
...
GSE171524_norm$TGFB_label = ""

for (i in 1:length(allcellname)){
  if (allcellname[i] %in% colnames(group1)){
    GSE171524_norm$TGFB_label[i] = "TGFB1high_CD14CD16-Mo"
  }
  else if (allcellname[i] %in% colnames(group2)){
    GSE171524_norm$TGFB_label[i] = "TGFB1low_CD14CD16-Mo"
  }
  else if (allcellname[i] %in% colnames(group3)){
    GSE171524_norm$TGFB_label[i] = "TGFB1low_CD14CD16-Mo"
  }
  else {
    GSE171524_norm$TGFB_label[i] = GSE171524_norm$label[i]
  }
}

# Select the TGFB1high_CD14CD16-Mo and TGFB1low_CD14CD16-Mo cells
monocyte_3 = subset(x = GSE171524_norm, subset = TGFB_label == "TGFB1high_CD14CD16-Mo" | TGFB_label == "TGFB1low_CD14CD16-Mo")
...
# This is the end of the TGFB1high monocyte in GSE171524
#################################################################################################################